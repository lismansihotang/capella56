...
(($invoicearno != '%%')?" and t.cashbankinid in (
  ...
  public function actionsearchdetail() {
    ...
    $row             = array();
    $footer             = array();
    $cmd             = Yii::app()->db->createCommand()->select('count(1) as total')->from('cashbankindetail t')->leftjoin('invoicear b', 'b.invoicearid=t.invoicearid')->leftjoin('currency c', 'c.currencyid=t.currencyid')->where("(cashbankinid = :cashbankinid)", array(
      ':cashbankinid' => $id
    ))->queryScalar();
    $result['total'] = $cmd;
    $cmd             = Yii::app()->db->createCommand()->select('t.*,b.invoicearno,c.currencyname,c.symbol')->from('cashbankindetail t')->leftjoin('invoicear b', 'b.invoicearid=t.invoicearid')->leftjoin('currency c', 'c.currencyid=t.currencyid')->where("(cashbankinid = :cashbankinid)", array(
      ':cashbankinid' => $id
    ))->offset($offset)->limit($rows)->order($sort . ' ' . $order)->queryAll();
		$symbol = '';
    foreach ($cmd as $data) {
    ...
    $row             = array();
    $footer             = array();
    $cmd             = Yii::app()->db->createCommand()->select('count(1) as total')->from('cashbankinjurnal t')->leftjoin('account a', 'a.accountid = t.accountid')->leftjoin('currency b', 'b.currencyid = t.currencyid')->leftjoin('company c', 'c.companyid = a.companyid')->where("(cashbankinid = :cashbankinid) and t.accountid > 0", array(
      ':cashbankinid' => $id
    ))->queryScalar();
    $result['total'] = $cmd;
	...
	private function ModifyData($connection,$arraydata) {
		...
		$sql     = 'call Modifcashbankin(:vid,:vplantid,:vcashbankindate,:vaccountid,:vaddressbookid,:vheadernote,:vcreatedby)';
  ...
  public function actionSave() {
	...
	private function ModifyDataDetail($connection,$arraydata) {
		$id = (isset($arraydata[0])?$arraydata[0]:'');
		if ($id == '') {
			$sql     = 'call Insertcashbankindetail(:vcashbankinid,:vinvoicearid,:vamount,:vcurrencyid,:vratevalue,:vdetailnote,:vcreatedby)';
			$command = $connection->createCommand($sql);
		} else {
			$sql     = 'call Updatecashbankindetail(:vid,:vcashbankinid,:vinvoicearid,:vamount,:vcurrencyid,:vratevalue,:vdetailnote,:vcreatedby)';
			$command = $connection->createCommand($sql);
			$command->bindvalue(':vid', $arraydata[0], PDO::PARAM_STR);
			$this->DeleteLock($this->menuname, $arraydata[0]);
		}
		...
		$command->bindvalue(':vratevalue', $arraydata[5], PDO::PARAM_STR);
		$command->bindvalue(':vdetailnote', $arraydata[6], PDO::PARAM_STR);
		$command->bindvalue(':vcreatedby', Yii::app()->user->name, PDO::PARAM_STR);
		$command->execute();
	}
  public function actionSavedetail() {
    ...
    try {
			$this->ModifyDataDetail($connection,array((isset($_POST['cashbankindetailid'])?$_POST['cashbankindetailid']:''),$_POST['cashbankinid'],
				$_POST['invoicearid'],$_POST['amount'],$_POST['currencyid'],$_POST['ratevalue'],$_POST['detailnote'],
			));
			$transaction->commit();
      GetMessage(false,getcatalog('insertsuccess'));
		}
  ...
  private function ModifyDataJurnal($connection,$arraydata) {
		$id = (isset($arraydata[0])?$arraydata[0]:'');
		if ($id == '') {
			$sql     = 'call Insertcashbankinjurnal(:vinvoicearid,:vaccountid,:vdebit,:vcredit,:vcurrencyid,:vratevalue,:vdetailnote,:vcreatedby)';
			$command = $connection->createCommand($sql);
		} else {
			$sql     = 'call Updatecashbankinjurnal(:vid,:vinvoicearid,:vaccountid,:vdebit,:vcredit,:vcurrencyid,:vratevalue,:vdetailnote,:vcreatedby)';
			$command = $connection->createCommand($sql);
			$command->bindvalue(':vid', $arraydata[0], PDO::PARAM_STR);
			$this->DeleteLock($this->menuname, $arraydata[0]);
		}
  ...
  public function actionPurgedetail() {
    ...
    if (isset($_POST['id'])) {
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call Purgecashbankindetail(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $id, PDO::PARAM_STR);
        $command->bindvalue(':vcreatedby', Yii::app()->user->name, PDO::PARAM_STR);
        $command->execute();
  ...
  public function actionPurgetax() {
    ...
    if (isset($_POST['id'])) {
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call Purgecashbankintax(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $id, PDO::PARAM_STR);
        $command->bindvalue(':vcreatedby', Yii::app()->user->name, PDO::PARAM_STR);
        $command->execute();
  ...
  public function actionGenerateinvar() {
		$sql = "select sum(ifnull(b.total,0)) as amount,b.currencyid,b.ratevalue,a.headernote
			from invoicear a
    ...
    if (Yii::app()->request->isAjaxRequest) {
      ...
      try {
        $sql     = '';
        $command = $connection->createCommand($sql);
        $command->bindvalue(':vid', $_POST['invoicearid'], PDO::PARAM_INT);
        $command->execute();
        $transaction->commit();
      }
      catch (Exception $e) {
    ...
    foreach ($dataReader as $row) {
      ...
      $this->pdf->row(array(
        ...
        ''
			...
			$sql1        = "select distinct ifnull(aa.amount,0) as biaya, ca.currencyname, aa.ratevalue, aa.detailnote, ba.invoicearno,ca.symbol 
