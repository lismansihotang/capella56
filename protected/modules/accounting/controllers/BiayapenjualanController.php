<?php
class BiayapenjualanController extends Controller {
  public $menuname = 'biayapenjualan';
  ...
  public function search() {
    ...
    $repbiayapenjualanid     = isset($_POST['repbiayapenjualanid']) ? $_POST['repbiayapenjualanid'] : '';
    ...
    $sort            = isset($_POST['sort']) ? strval($_POST['sort']) : 'repbiayapenjualanid';
    ...
    $cmd             = Yii::app()->db->createCommand()->select('count(1) as total')
      ...
      ':repbiayapenjualanid' => '%' . $repbiayapenjualanid . '%',
      ':companyid' => '%' . $companyid . '%',
    ...
    foreach ($cmd as $data) {
      $row[] = array(
        'repbiayapenjualanid' => $data['repbiayapenjualanid'],
  ...
  private function ModifyData($connection,$arraydata) {
    ...
    if ($id == '') {
      $sql     = 'call Insertbiayapepenjualan(:vcompanyid,:vaccountcode,:vplantcode,:vnourut,:visbold,:viscount,:vcounttype,:vketerangan,:vcreatedby)';
      $command = $connection->createCommand($sql);
    } else {
      $sql     = 'call Updatebiayapenjualan(:vid,:vcompanyid,:vaccountcode,:vplantcode,:vnourut,:visbold,:viscount,:vcounttype,:vketerangan,:vcreatedby)';
      $command = $connection->createCommand($sql);
  ...
  public function actionSave() {
    ...
    try {
      $this->ModifyData($connection,array((isset($_POST['repbiayapenjualanid'])?$_POST['repbiayapenjualanid']:''),$_POST['companyid'],
				$_POST['accountcode'],$_POST['plantcode'],$_POST['nourut'],$_POST['isbold'],$_POST['iscount'],$_POST['counttypeid'],
        $_POST['keterangan']));
      $transaction->commit();
  ...
  public function actionPurge() {
    ...
    if (isset($_POST['id'])) {
      ...
      $transaction = $connection->beginTransaction();
      try {
        $sql     = 'call Purgebiayapenjualan(:vid,:vcreatedby)';
        $command = $connection->createCommand($sql);
    ...
    parent::actionDownload();
		...
		$sql = "delete from repbiayapenjualanlajur where companyid = ".$companyid."  
      and tahun = year('".$tgl."')
      and bulan = month('".$tgl."')";
		Yii::app()->db->createCommand($sql)->execute();
		$sql = "select * from repbiayapenjualan where companyid = " . $companyid;
		...
		foreach($datareader as $data) {
			...
			foreach($plants as $plant) {
				if ($data['counttypeid'] == 2) {
					...
					$sql1 = "insert into repbiayapenjualanlajur (companyid,bulan,tahun,nourut,keterangan,plantcode,plantvalue,accountcode,iscount,isbold) 
  ...
  public function actionDownPDF() {
    ...
    $this->pdf->Cell(0, 0, GetCatalog('biayapenjualan'), 0, 0, 'C');
    $this->pdf->Cell(-277, 10, 'Per : ' . date("d F Y", strtotime($_GET['date'])), 0, 0, 'C');
    $i = 0;
		
		foreach ($datareader as $data) {
			$sql1 = "select plantcode from repbiayapenjualan where companyid = ".$_GET['company']." limit 1";
		...
		foreach ($datareader as $data) {
			$sql1 = "select distinct plantcode from repbiayapenjualan where companyid = ".$_GET['company']." limit 1";
			$datareader1 = $this->connection->createCommand($sql1)->queryScalar();
			$plants = explode(';',$datareader1);
			$total = 0;
			if ($data['nourut'] == 99) {
