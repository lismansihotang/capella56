...
<table id="dg-profitloss" style="width:100%;height:95%">
</table>
<div id="tb-profitloss">
	...
	<?php if (CheckAccess($this->menuname, $this->isdownload) == 1) {  ?>
		<select class="easyui-combogrid" id="dlg_search_profitloss_companyid" name="dlg_search_companyid" style="width:250px" data-options="
								panelWidth: 500,
								required: true,
								idField: 'companyid',
		...
		<input class="easyui-datebox" type="text" id="dlg_search_profitloss_bsdate" name="dlg_search_pldate" data-options="formatter:dateformatter,required:true,parser:dateparser,prompt:'As of Date'"></input>
		<a href="javascript:void(0)" title="Generate PL Sebelum Pajak"class="easyui-linkbutton" iconCls="icon-bom" plain="true" onclick="generateplsebelumpajak()"></a>
		<a href="javascript:void(0)" title="Generate PL"class="easyui-linkbutton" iconCls="icon-bom" plain="true" onclick="generatepl()"></a>
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-pdf" plain="true" onclick="downpdfprofitloss()"></a>
	<?php }?>
...
$('#dg-profitloss').edatagrid({
	...
	ctrlSelect:true,
	autoSave:false,
	url: '<?php echo $this->createUrl('profitloss/index',array('grid'=>true)) ?>',
	saveUrl: '<?php echo $this->createUrl('profitloss/save',array('grid'=>true)) ?>',
	updateUrl: '<?php echo $this->createUrl('profitloss/save',array('grid'=>true)) ?>',
	destroyUrl: '<?php echo $this->createUrl('profitloss/purge',array('grid'=>true)) ?>',
...
function generateplsebelumpajak() {
	$('.ajax-loader').css('visibility', 'visible');
	jQuery.ajax({'url':'<?php echo $this->createUrl('profitloss/generateplsebelumpajak') ?>',
		'data':{			
			'companyid':$('#dlg_search_profitloss_companyid').combogrid('getValue'),
			'bsdate':$('#dlg_search_profitloss_bsdate').datebox('getValue')},
...
function generatepl() {
	$('.ajax-loader').css('visibility', 'visible');
	jQuery.ajax({'url':'<?php echo $this->createUrl('profitloss/generatepl') ?>',
		'data':{			
			'companyid':$('#dlg_search_profitloss_companyid').combogrid('getValue'),
			'bsdate':$('#dlg_search_profitloss_bsdate').datebox('getValue')},
		'type':'post','dataType':'json',
...
function downpdfprofitloss() {
	...
	var companyid = $('#dlg_search_profitloss_companyid').combogrid('getValue');
	var pldate = $('#dlg_search_profitloss_bsdate').datebox('getValue');
	window.open('<?php echo $this->createUrl('profitloss/downpdf') ?>?company='+companyid+'&date='+pldate+'&per='+10);
}
